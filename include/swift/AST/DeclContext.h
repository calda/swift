...
public:
  ...
  /// Returns the innermost context that is a ClosureExpr, which defines how self behaves,
  /// unless within a type context that redefines self.
  LLVM_READONLY
  DeclContext *getInnermostClosureForSelfCapture();
  const DeclContext *getInnermostClosureForSelfCapture() const {
    return
        const_cast<DeclContext *>(this)->getInnermostClosureForSelfCapture();
  }
