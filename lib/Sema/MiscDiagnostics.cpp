...
static void diagnoseImplicitSelfUseInClosure(const Expr *E,
  ...
  public:
    ...
    static bool selfDeclAllowsImplicitSelf(const ValueDecl *selfDecl,
      ...
      //  - Even if this isn't a closure with a `weak self` capture, it's
      //    possible this is a nested closuree that unwraps its parent's
      //    weak self capture, so we should still validate the rebinding.
      ...
      if (!ctx.LangOpts.isSwiftVersionAtLeast(6) && closureHasWeakSelfCapture(inClosure) && !hasValidSelfRebinding(parentConditionalStmt(selfDecl), inClosure)) {
        return false;
      }
      ...
      if (auto autoclosure = dyn_cast<AutoClosureExpr>(inClosure)) {
        ...
        auto isThunk = autoclosure->getThunkKind() != AutoClosureExpr::Kind::None;
        if (isThunk) {
          return true;
        }
        ...
        if (auto parent = parentClosure(inClosure)) {
          if (closureHasWeakSelfCapture(inClosure) && !hasValidSelfRebinding(parentConditionalStmt(selfDecl), inClosure)) {
            return false;
          }
        }
      }
    ...
    /// The `LabeledConditionalStmt` that contains the given `ValueDecl` if present
    static LabeledConditionalStmt *parentConditionalStmt(const ValueDecl *selfDecl) {
      if (!selfDecl) {
        return nullptr;
      }
    ...
    /// Determines whether or not this is a simple self capture by retreiving the
    /// `CaptureListEntry` that contains the `selfDecl`.
    ...
    static const ClosureExpr *parentClosure(const AbstractClosureExpr *closure) {
      ...
      return dyn_cast_or_null<const ClosureExpr>(parentContext->getInnermostClosureForSelfCapture());
    }
    ...
    PreWalkResult<Expr *> walkToExprPre(Expr *E) override {
      ...
      auto shouldOnlyWarn = [&](Expr *selfRef) {
        ...
        if (closureHasWeakSelfCapture(ACE)) {
          ...
          if (auto closureExpr = dyn_cast<ClosureExpr>(ACE)) {
            if (auto capturedSelfDecl = closureExpr->getCapturedSelfDecl()) {
              if (closureHasWeakSelfCapture(closureExpr) && !isSimpleSelfCapture(capturedSelfDecl)) {
                return true;
              }
            }
          }
          ...
          if (auto conditionalStmt = parentConditionalStmt(selfDecl)) {
            auto isValidSelfRebinding = hasValidSelfRebinding(conditionalStmt, ACE);
            ...
            // In Swift 5.8 - 5.10 we used `requiredLoadExpr` instead
            ...
            // until Swift 6.
            auto legacyRebindsSelfResult = conditionalStmt->rebindsSelf(
                                             ACE->getASTContext(),
        ...
        if (auto parentContext = ACE->getParent()) {
          if (auto parentClosure = dyn_cast_or_null<ClosureExpr>(parentContext->getInnermostClosureForSelfCapture())) {
